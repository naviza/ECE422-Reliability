...
if len(sys.argv) < 4:
    ...
    swarm_master_ip = '10.2.12.118'  # ip address of the Swarm master node
...
redisClient = redis.StrictRedis(host=swarm_master_ip, port=6379)
...
def workload(user):
    while True:
        ...
        try:
            ...
            redisClient.rpush('bruhtestlist', str(t1 - t0))
        except:
            print("Error occured for " + user + ". Retrying...")
            time.sleep(think_time)
